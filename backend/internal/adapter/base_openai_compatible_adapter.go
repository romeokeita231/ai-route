...
func (a *BaseOpenAICompatibleAdapter) invoke(
    model *entity.Model,
    ...
    if chatRequest.EnableReasoning != nil && *chatRequest.EnableReasoning {
        if model.SupportReasoning == 1 {
            ...
            log.Printf("reasoning requested but model does not support: model=%s provider=%s",
                model.ModelKey, provider.ProviderName)
        }
    }
    if stream {
